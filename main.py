    ...
    while running:
        ...
        if not game_over and not human_turn and not move_undone:
            if not ai_thinking:
                ...
                move_finder_process = Process(target=randomAI.findBestMove, args=(game_state, valid_moves, return_queue))
                move_finder_process.start()

            if not move_finder_process.is_alive():
                ai_move = return_queue.get()
                if ai_move is None:
                    ai_move = randomAI.findRandomMove(valid_moves)
